...
namespace commercetools.Core.Tests
{
    ...
    public class CartManagerTest : IDisposable
    {
        ...
        public async Task ShouldCreateAndDeleteCartInExternalAmountTaxModeAndTestExternalAmountTaxModeOrientedActions()
        {
            ...
            Cart cart = response.Result;
            Assert.True(cart.TaxMode == TaxMode.ExternalAmount, string.Format("Incorrect TaxMode: {0}", cart.TaxMode));
            Assert.NotNull(cart.Id);
            Assert.NotNull(cart.CustomLineItems);
            Assert.True(cart.CustomLineItems.Count > 0);
            ...
            Assert.True(response.Success, "SetCustomLineItemTaxAmountAction Failed");
            cart = response.Result;
            Assert.True(cart.CustomLineItems[0].TaxRate.Amount == subRate.Amount, string.Format("Mismatching Tax Rates: {0} vs {1}", cart.CustomLineItems[0].TaxRate.Amount, subRate.Amount));
            ...
            Assert.True(response.Success, "AddLineItem Failed");
            cart = response.Result;
            ...
            response = await _client.Carts().UpdateCartAsync(cart, new AddLineItemAction(_testProduct.Id, _testProduct.MasterData.Current.MasterVariant.Id) { Quantity = 1 });
            Assert.True(response.Success, "SetLineItemTaxAmountAction Failed");
            cart = response.Result;
            ...
            response = await _client.Carts().UpdateCartAsync(cart, customLineItemTaxAmountAction);
            Assert.True(response.Success, "SetShippingMethodTaxAmountAction Failed");
            cart = response.Result;
            ...
            Assert.True(response.Success, "SetCartTotalTaxActionResponse Failed");
            cart = response.Result;

            string deletedCartId = cart.Id;

            response = await _client.Carts().DeleteCartAsync(cart);
            Assert.True(response.Success, "Delete Cart Failed");

            cart = response.Result;

            response = await _client.Carts().GetCartByIdAsync(deletedCartId);
            Assert.False(response.Success, "Confirm Delete Cart Failed");
        }
